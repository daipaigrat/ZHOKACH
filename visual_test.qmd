...
```{r}
data_test <- read_from_drive("analysis_data.csv", folder_id)
```
...
#    ))
data <- read_from_drive("analysis_data.csv", folder_id)
```
...
```{r}
prepare_sentiment_data <- function(df) {
  df %>%
    mutate(
      date = as.Date(date),  # Преобразуем в дату
      sentiment = factor(sentiment, levels = c("negative", "neutral", "positive", "unknown"))
    ) %>%
    count(date, sentiment, name = "count") %>%
...
```{r}
prepare_topic_stats <- function(df) {
  df %>%
    count(topic, name = "count") %>%
    mutate(
  ...
  fluidRow(
    column(
      ...
      div(class = "content-box",
          ...
          plotlyOutput("distr_plot", height = "300px")
      )
    ),
    column(
      width = 6,
...
server <- function(input, output) {
  ...
  output$distr_plot <- renderPlotly({
    ggplotly(plot_distr(daily_stats))
  })
  ...
  output$cat_table <- renderDT({
    stats <- prepare_topic_stats(data)
    ...
    datatable(
      ...
      rownames = FALSE,
      extensions = c('Scroller'),  # Добавляем расширение для плавной прокрутки
      options = list(
        deferRender = TRUE,
        scrollY = 400,            # Фиксированная высота области прокрутки
        scroller = TRUE,
        scrollX = TRUE,           # Горизонтальная прокрутка при необходимости
        pageLength = nrow(stats), # Показываем все строки (все категории)
        lengthChange = FALSE,     # Отключаем выбор количества строк
        dom = 't',               # Отключаем пагинацию и другие элементы управления
        language = list(
      ...
      formatStyle(
        'Доля, %',
        background = styleColorBar(c(0, max(stats$percentage)), 'lightblue'),
        backgroundSize = '98% 88%',
        backgroundRepeat = 'no-repeat',
        backgroundPosition = 'center'
      ) %>%
      formatRound('Доля, %', digits = 1) %>%
      formatCurrency('Количество сообщений', currency = "", digits = 0, mark = " ")
  })
}
```
...
output$active <- renderPlotly({
    activity_data <- prepare_activity_data(data, input$time_unit)
    
    plot_ly(activity_data, x = ~time, y = ~message_count, 
            type = 'scatter', mode = 'lines', 
            fill = 'tozeroy', fillcolor = 'rgba(100, 149, 237, 0.4)',
            line = list(color = 'rgb(100, 149, 237)'),
            hoverinfo = 'text',
            text = ~paste0("Дата: ", format(time, "%Y-%m-%d %H:%M"), 
                          "<br>Сообщений: ", message_count)) %>%
      layout(
