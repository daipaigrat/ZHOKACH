...
server <- function(input, output, session) {
  ...
  show_dashboard_modal <- function(html_file) {
...
'
  ...
  observeEvent(input$start_btn, {
    if (!is_running()) {
      ...
      chat <- preprocess_data(members_path, chat_path)
      write.csv(member, file = "processed_members.csv", fileEncoding = "UTF-8", row.names = FALSE)
      write.csv(chat, file = "processed_chat_data.csv", fileEncoding = "UTF-8", row.names = FALSE)
      #source(LLM.R)
